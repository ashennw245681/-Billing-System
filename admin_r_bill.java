 ...
 * @author Blocker
 */
class admin_r_bill extends JFrame{
...
JTable table = new JTable();
 ...
 Object[] columns = {"Return ID","Bill No","Total Amount","Items","Date","Time"};
...
Object[] row = new Object[6];
           ...
           try {
                    ...
                    ResultSet rs = st.executeQuery("SELECT * FROM return_no ");

                    while (rs.next()) {

                        row[0] = rs.getString("Return_id");
                        row[1] = rs.getString("Bill_no");
                        row[2] = rs.getString("Total");
                        row[3] = rs.getString("Items");
...
bill.setBounds(0, 0, 0, 0);
 ...
 Object[] columns = {"Return ID","Bill No","Total Amount","Items","Date_Time"};
	DefaultTableModel model = new DefaultTableModel();
	model.setColumnIdentifiers(columns);   
...
item.setBounds(0, 0, 0, 0);
 ...
 Object[] columns = {"Return ID","Item ID","Name & Bill No","Total","Quantity","Total Amount","Date","Time"};   
...
Object[] row = new Object[8];
           ...
           try {
                    ...
                    Connection con = DriverManager.getConnection("jdbc:mysql://localhost/Super_market_system", "root", "");
                    Statement st = con.createStatement();
                    ResultSet rs = st.executeQuery("SELECT * FROM return_save ");

                    while (rs.next()) {

                        row[0] = rs.getString("Return_id");
                        row[1] = rs.getString("Item_id");
                        row[2] = rs.getString("Bill_no");
...
JOptionPane.showMessageDialog(null, "Logout","Logout success",JOptionPane.WARNING_MESSAGE);                          
 ...
 JLabel nn = new JLabel ("Return Bill");
        nn.setBounds(160,20,300,33);
